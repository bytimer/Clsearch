...
; [IPC方式和everything进行通讯，修改于AHK论坛] https://www.autohotkey.com/boards/viewtopic.php?t=104445 v2 定义Everything类的示例

/* ; 使用===========
	#Include Everything.ahk
	
	evObj := Everything() ; Everything("path to Everything64.dll ")
	;EvIsAdminStatus:=evObj.GetIsAdmin() ? "管理员权限" : "非管理员"
	;evObj.SetSearch("regex:\d{30}") ; 正则表达式搜索会慢一些
	evObj.SetSearch("cmd.exe")
	evObj.SetMax(3) ; 限制返回的最大结果数
	evObj.SetSort(14) ; 14=修改时间_降序
	evObj.SetMatchWholeWord() ; 文件名匹配完整单词 是ww: 而非wfn:
	evObj.SetRequestFlags(4) ; 获取完整路径和文件名 =默认值3
	evObj.Query()
	if evObj.GetNumResults() > 0
		ToolTip evObj.GetResultFullPathName(0) ; 结果第一项的完整路径
	evObj := ""
*/
...
class Everything
{
    ...
    __New(dllPath := "Everything64.dll")
    {
        if FileExist(dllPath)
            this.dll := dllPath
        else if FileExist(A_LineFile "\..\Everything64.dll") ; 如果作为 lib 被 #Include
            this.dll := A_LineFile "\..\Everything64.dll"
        else {
            msgbox("DLL未找到")
            this.error := Error("未找到Everything SDK dll文件，创建对象失败")
            this.error.Message .= "`n函数 : Everything()"
            this.error.What := "__New"
            throw (this.error)
		}
        ;this.hModule := DllCall("LoadLibrary", "str", this.dll, "Ptr") ; 已用#DllLoad加载dll，这里无须
        this.dll .= "\"
    }
    /*
	...
	SetSearch(aValue){
		dllcall(this.dll "Everything_SetSearch", "Str", aValue)
	}
	*/
	;释放库分配的任何内存
	...
	; 11=创建时间_升序  12=创建时间_降序;  13=修改时间_升序  14=修改时间_降序;  19=运行次数_升序  20=运行次数_降序
	...
	;设置请求标志 (4=请求完整文件路径)
	SetRequestFlags(dwRequestFlags := 0x00000004) => DllCall(this.dll "Everything_SetRequestFlags", "int", dwRequestFlags)

	;使用当前搜索状态执行 Everything IPC 查询  执行搜索动作
    ...
    ;返回文件结果的总数 使用Everything_SetRequestFlags 或 Everything_SetSort 时不支持
    Everything_GetResultRunCount => (*) => this._GetNum("Everything_GetTotFileResults")
    ...
    ;返回可见结果中指定索引对应文件的运行次数 0x00000400
	...
	;设置“按文件名索引的所有内容”中指定文件的运行计数。该文件不必存在，创建文件后，它将具有正确的运行历史记录。
    ...
    ;检查指定的文件信息属性列是否已编制索引 ; https://www.voidtools.com/support/everything/sdk/everything_isfileinfoindexed/
    ;fileInfoType 索引类型：1文件大小，2文件夹大小，3创建日期，4修改日期，5访问日期，6属性
    ...
    ;返回可见结果的路径部分(不含文件名)  比Everything_GetResultFullPathName 快
    ...
    ;返回预设定的排序方式 https://www.voidtools.com/support/everything/sdk/everything_getsort/
    ...
    ;比如v1.5.0.1396，Major=1，Minor=5，BuildNumber=1396，Revision为空
    ;检索 Everything 的主版本号
